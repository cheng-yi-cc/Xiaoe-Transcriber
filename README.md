...
- 使用 `llama.cpp` 在本机做归纳式总结，并校验生成内容是否得到逐字稿支持
...
- 首次下载量取决于所选模型组合：共用引擎约 1.5 GB，Whisper Small/Medium/Large 约 0.5/1.5/1.6 GB，总结模型 Qwen3-8B 约 5 GB、Qwen2.5-1.5B 约 1.1 GB
...
6. 在设置页分别选择转写模型和总结模型：点击未安装的模型行，确认下载量后自动开始下载；已安装的模型点击即切换使用。可以安装多个模型共存，不再需要的非当前模型可点行内“删除”释放空间。安装包本身不包含模型，也不会在安装应用时自动下载模型。
...
- 自动总结默认推荐 Qwen3-8B 本地模型（约 5GB 下载，建议 8GB 显存以完整载入，低显存可选 1.5B 档）；应用会拦截明显幻觉并使用提取式摘要兜底，但仍不适合作为高精度出版稿。
