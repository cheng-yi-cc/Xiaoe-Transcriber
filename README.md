...
- 总结以连贯的散文段落概括直播内容，不套模板标题、不罗列高频词
- 总结出现大量无原文依据的用词或编造数字时，自动切换为逐字稿原文代表性片段摘录
...
└─ 视频标题_20260819_213045/
   ├─ 总结.md
   ├─ 完整文字稿.md
   └─ 总结-模型原始输出（未通过校验）.md   ← 仅在总结未通过可靠性校验时生成
```
...
1. 从 GitHub Releases 下载最新的 `Xiaoe-Transcriber-Setup-*.exe`。
...
8. 进入工作台，确认当前转写模型和总结模型，粘贴单视频回放页链接并开始转写。应用会在任务开始前复检会话。任务运行期间可以继续进入设置页修改配置；当前任务不受影响，新配置从下一次任务开始使用。为避免破坏运行中的任务，该任务正在使用的模型需等任务结束后才能删除。
