...
- 所有首次下载的二进制和模型必须使用清单中的固定 SHA-256 校验；两个例外：VC++ 运行库从微软官方 `https://aka.ms/vs/17/release/vc_redist.x64.exe` 实时下载，依赖微软签名；应用内更新从 GitHub Releases 实时下载新版安装包（`api.github.com` 与 `github.com` 官方域名，HTTPS 传输），依赖传输安全与文件大小校验。
